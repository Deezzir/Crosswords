...
<template>
    <div class="mx-auto mt-2 h-full px-[5vw] sm:px-[10vw]">
        <Transition
            ...
            appear>
            <div v-if="!store.getCurGame" class="flex w-full flex-grow flex-col items-center">
                <div class="mb-4">
                    <h1 class="text-4xl font-bold text-gray-700 dark:text-white">Choouse a game to play</h1>
                </div>
                <div class="space-x-6">
                    <button
                        v-for="game in store.gameList"
                        :key="game.id"
                        class="min-w-[10vw] rounded bg-blue-700 px-4 py-2 text-2xl font-bold text-white hover:bg-blue-500"

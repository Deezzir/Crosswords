...
<template>
    <div class="mt-12 grid h-full w-full grid-cols-1 content-between gap-3 xl:mt-0 xl:gap-8">
        ...
        <div class="grid grid-cols-9 rounded-lg bg-slate-300 xl:grid-cols-3 xl:gap-3 xl:rounded-none xl:bg-transparent">
            <button
                v-for="n in 9"
                :key="n"
                class="rounded-md p-6 text-4xl text-blue-500 transition-colors duration-300 hover:bg-gray-300 dark:text-blue-600 dark:hover:bg-gray-400 xl:bg-slate-200 xl:dark:bg-slate-300">
                {{ n }}
            </button>
        </div>
        <div class="flex">
